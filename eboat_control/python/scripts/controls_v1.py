...
class Sailor(wx.Frame):
    def __init__(self):
        ...
        self.reward = 10.0
        self.rewardDisplay = wx.TextCtrl(pnl,
                                         value = f"{self.reward}",
                                         style = wx.TE_READONLY)
        #--> ELEMENTS TO DISPLAY SENSOR DATA
        distanceFromWayPoint = wx.StaticText(pnl, label="Distance from the Goal: {} m".format(0))
        trajectoryAngle      = wx.StaticText(pnl, label="Trajectory Angle      : {} m".format(0))
        linearVelocity       = wx.StaticText(pnl, label="Boat linear velocity  : {} m/s".format(0))
        apparentWindAngle    = wx.StaticText(pnl, label="Apparent wind angle   : {} degrees".format(0))
        apparentWindSpeed    = wx.StaticText(pnl, label="Apparent wind speed   : {} m/s".format(0))


        hsizer.Add(trueWindSTR, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALIGN_LEFT | wx.LEFT, 5)
        hsizer.Add(self.trueWindVec, 0, wx.ALL | wx.LEFT, 5)
        hsizer.Add(self.rewardDisplay, 0, wx.ALL | wx.LEFT, 5)
        vsizer.Add(hsizer)
        vsizer.AddSpacer(30)
        ...
        self.reward = self.getObservations()[0]
