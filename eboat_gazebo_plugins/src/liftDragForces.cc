...
void LiftDragForces::OnUpdate()
{
  ...
  if (this->waterVelVec.Length() > 0)
  {
    ...
    /*if (this->rudderLink->RelativePose().Rot().Yaw() < 0)
    {
      this->liftDirection *= -1; // impose symmetry
    }*/
    ...
    this->coefIndex = (this->atkangle - iatk) < 0.5 ? iatk
                                                    : iatk + 1;
    this->lift  = this->q * 2.0*rudderCL[this->coefIndex] * this->liftDirection;  //-> the lift force is normal to the lift-drag plan
    this->drag  = this->q * 2.0*rudderCD[this->coefIndex] * this->dragDirection; //-> the drag
    this->force = this->lift + this->drag;
    //////////////////////////////////////////////////////////
