...
void SailPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  GZ_ASSERT(this->boomLink, "SailPlugin boom link pointer is NULL");
  ...
  GZ_ASSERT(this->boomLink, "SailPlugin base link pointer is NULL");
  ...
  if (_sdf->HasElement("cp"))
    ...
    this->cp = ignition::math::Vector3d(0.7,0,1.5);
...
void SailPlugin::Init()
{
  ...
  this->armLength = (((this->boomLink->WorldCoGPose().Pos() + this->cp) - this->model->GetJoint("boom_joint")->WorldPose().Pos())*this->forward).Length();
...
void SailPlugin::OnUpdate()
{
  ...
  if (this->aparentWindVelVec.Length() > 0)
  {
    ...
    ignition::math::Vector3d chordLine = this->boomLink->WorldPose().Rot().RotateVector(-forward);
    //->Upward direction in the World frame
    ignition::math::Vector3d upwardW   = this->boomLink->WorldPose().Rot().RotateVector(upward);
    ...
    ignition::math::Vector3d dynamic_cp = (this->boomLink->WorldCoGPose().Pos() + this->boomLink->WorldCoGPose().Rot().RotateVector(this->cp)) - this->baseLink->WorldCoGPose().Pos();
    ...
    double torque = (alpha < 90) ? (force.Dot(normal)*normal).Length()*armLength*(-1) // Negative
                                 : (force.Dot(normal)*normal).Length()*armLength;     // Positive
