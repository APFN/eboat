...
class GazeboOceanEboatEnv(gazebo_env.GazeboEnv):

	def __init__(self):
		self.EBOAT_HOME = "/eboat_ws/src/eboat_gz_1"
		...
		# -->GLOBAL VARIABLES
		self.DMAX = 25.0  # --> Threshold for distance. If the boat goes far than that, a done signal is trigged.
		self.D0 = None  # --> The intial distance from the waypoint

		#--> Eletric propulsion [-5,5], Rudder angle [-60,60], Boom angle [0, 90]
		self.action_space = spaces.MultiDiscrete([np.arange(-5,6).shape[0] ,   #-->eletric propulsion speed
												  np.arange(0, 91).shape[0],   #-->boom angle    (retranca)
												  np.arange(-60,61).shape[0]]  #-->rudder angle  (leme)
												 )

		self.observation_space = spaces.Box(low   = np.array([0.0  , -180.0, 0.0 , 0.0 , -180.0], dtype=float),
											high  = np.array([5.0e2, 180.0 , 30.0, 30.0, 180.0 ], dtype=float),
											dtype = float
										   )
	...
	def step(self, action):
		...
		self.pvel_pub.publish(action[0]-5)
		self.bang_pub.publish(action[1])
		self.rang_pub.publish(action[2]-60)
		...
		return observations[:5], reward, done, False, {}
